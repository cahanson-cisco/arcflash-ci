...
resource "intersight_fabric_server_role" "server_ports-A" {
  for_each = toset([for p in range(intersight_fabric_port_mode.fibrechannel_ports-A.port_id_end + 1, 16 + 1) : tostring(p)])
...
resource "intersight_fabric_server_role" "server_ports-B" {
  for_each = toset([for p in range(intersight_fabric_port_mode.fibrechannel_ports-B.port_id_end + 1, 16 + 1) : tostring(p)])
